# Standalone dev / test / build flake for the goodnet-link-quic
# plugin. See `plugins/links/tcp/flake.nix` for the canonical
# pattern; this flake is a copy with plugin-specific knobs swapped in.
#
# goodnet-standalone-plugin: quic
{
  description = "GoodNet link plugin: quic — standalone plugin flake.";
    ...
    in
    {
      ...
      devShells = forAllSystems (system: pkgs: {
        default = helpers.mkPluginDevShell pkgs {
          ...
          welcomeText = ''
  goodnet-link-quic  —  standalone plugin dev shell
    ...
    nix run .#debug      — Debug build + gdb on test_quic
'';
        };
      });
